...
const projects = [
  {
    ...
    client: "LivelyCura",
    summary:
      "A healthcare operations platform with pharmacy, laboratory, courier, customer, and admin workflows.",
    problem:
      "Healthcare teams need operational tools that reduce manual work and keep pharmacy/lab workflows responsive.",
  ...
  },
  {
    ...
    client: "Bora Capital",
    summary:
      "A finance platform backend and mobile experience for investment, accounts, goals, transactions, and user onboarding.",
    problem:
      "The product needs reliable financial workflows, secure user onboarding, and integrations with external services.",
  ...
  },
  {
    ...
    client: "PizzaMan",
    summary:
      "Operational admin and mobile tooling for restaurant workforce, call-center, and internal reporting workflows.",
  ...
  },
  {
    ...
    slug: "homepal",
    summary:
      "A property listing platform for buying, renting, and managing real estate listings.",
    problem:
      "Property seekers and managers need a searchable platform that keeps listing discovery and management simple.",
  ...
  },
  {
    ...
    slug: "findjobs",
    summary:
      "A job tracking backend and frontend for managing applications with authentication, search, and OTP verification.",
    role: "Full-stack developer",
  ...
  },
  {
    ...
    summary:
      "A centralized dashboard for business data processing, workflow visibility, and team collaboration.",
...
export const seedDatabase = async () => {
  ...
  if (await ProfileModel.countDocuments()) await ProfileModel.deleteMany({});

  await ProfileModel.create({
    fullName: "Caleb Asakwin Azumah",
    title: "Software Engineer",
    headline:
      "I build scalable backend systems, modern web portals, and business-facing products with TypeScript, React, GraphQL, and MongoDB.",
    ...
    seo: {
      ...
      description:
        "Portfolio of Caleb Azumah, a software engineer building backend systems, client portals, admin dashboards, and business software.",
      keywords: [
        "Caleb Azumah",
        "Software Engineer",
        "React",
  ...
  if (await AvailabilityStatusModel.countDocuments())
    await AvailabilityStatusModel.deleteMany({});

  await AvailabilityStatusModel.create({
    label: "Open to software engineering roles and selected client projects",
    description:
      "Available for backend, frontend, full-stack, dashboard, and portal work.",
    acceptingWork: true,
  });

  if (await ExperienceModel.countDocuments()) await ExperienceModel.deleteMany({});

  await ExperienceModel.bulkWrite(
    [
      {
  ...
  if (await EducationModel.countDocuments()) await EducationModel.deleteMany({});
  ...
  if (await CertificationModel.countDocuments()) await CertificationModel.deleteMany({});
  ...
  if (await SkillModel.countDocuments()) await SkillModel.deleteMany({});

  await SkillModel.bulkWrite(
    skills.map(([name, category], order) => ({
      updateOne: {
        filter: { name },
  ...
  if (await ResumeAssetModel.countDocuments()) await ResumeAssetModel.deleteMany({});

  await ResumeAssetModel.bulkWrite(
    [
      {
  ...
  if (await TestimonialModel.countDocuments()) await TestimonialModel.deleteMany({});
    ...
    await TestimonialModel.create({
      ...
      company: "Portfolio",
      quote:
        "Testimonials can be added from the admin portal as client and teammate feedback becomes approved for publication.",
      isVisible: true,
      order: 1,
    });
  } catch (error) {
